...
import type { ChatMessage, Participant } from "./MPP";
...
export class Data {
    ...
    public static async updateUser(data: User) {
        return await update(this.getUserKey(data._id), (old: User | undefined) => {
            if (!old) {
    ...
    public static async addNameHistory(_id: string, name: string, date: number = Date.now()) {
